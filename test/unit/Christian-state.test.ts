...
describe('ChristianState - main', () => {
    ...
    describe("modifyChurchCommunityState", () => {
        it("should get reverted when Account is not an ADMINISTRATOR", async () => {
            const account1 = await christianStateContract.connect(accounts[1]);

            await expect(
                account1.modifyChurchCommunityState(churchCommunityContractAddress, true)
